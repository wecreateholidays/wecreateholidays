        ...
        stage('Deploy'){
            steps {
                withCredentials([string(credentialsId: 'AWS_ACCESS_KEY_ID', variable: 'AWS_ACCESS_KEY_ID'),string(credentialsId: 'AWS_SECRET_ACCESS_KEY', variable: 'AWS_SECRET_ACCESS_KEY')]) { //set SECRET with the credential content
                    ...
                    sh "echo \"Printing env variable in deploy step: ${params.ENV}\""
                    sh "ansible-playbook ./ansible-playbooks/we_create_holidays.yaml -i we_create_holidays.aws_ec2.yaml --extra-vars \"env=beta tagEnv=wecreateholidays_beta\""
